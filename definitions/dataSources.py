...
callSource = PostgreSQLSource(
    name="callSource",
    query="select call_data.id as id, closing_time as timestamp, round(extract(seconds from pickup_time-arrival_time)) as wait_duration, round(extract(seconds from closing_time-pickup_time)) as service_duration, is_solved::int, is_happy::int, is_predicted_to_be_happy::int as hapiness_prediction from call_data inner join customer on customer.id=call_data.customer_id",
